...
export class SearchPublisherComponent implements OnInit, OnDestroy {
  ...
  constructor(private dataService: DataService, private loaderService: LoaderService, private usersService: UsersService) {
    this.subscrp();
  }
  ngOnDestroy(): void {
  ...
  getPublihersByAssignment() {
    ...
    this.usersService.getPublihersByAssignment(this.assignmentType, this.congregation.id).subscribe((data) => {
